...
library(pacman)
p_load(tidyverse, magrittr, scales, patchwork, kableExtra, grid)
```
...
table_1 %>% 
  ...
  footnote(symbol = c("Model-predicted number of all-cause deaths from the five preceding years (95% credible interval, rounded to the next 100).",
                      ...
                      "First six months of the year only included in the analyses. No age specific data available for this time period for Sweden."))
```


## Table 2

Three measures of excess (all from main, age adjusted NB model with observed population counts) and years life lost.   
...
table_2 <- 
  ...
  mutate(rate_cri = str_c("(", Rate_lci, " to ", Rate_uci, ")")) %>%   relocate(rate_cri, .after = Rate_excess) %>% 
  ...
  left_join(
    ...
    results_age %>% 
      ...
      mutate(Lifeyearlost70 = number(round(yearly_excess_grouped_lifelost70/1000),accuracy=5L,big.mark=","),
             Lifeyearlost70_CI = paste0("(",
                                        number(yearly_excess_grouped_lifelost70_lower/1000,accuracy=5L,big.mark=","),
                                        " to ",
                                        number(yearly_excess_grouped_lifelost70_upper/1000,accuracy=5L,big.mark=","),")"))  %>% 
...
table_2 %>% 
  ...
  mutate(percent_cri = if_else(Year==2020,paste0(percent_cri,"$^§$"),percent_cri)) %>% 
  select(-Year) %>% 
  kbl(align = "lrrrrrrrr",
      col.names = c("Country", 
                    "Count", "95% CrI",
  ...
  add_header_above(c(" " = 1, 
                     ...
                     "Life years lost until 70$^‡$" = 2)) %>% # FIXME: dollar signs appear in the heads
  pack_rows("1890", 1, 2) %>%
  pack_rows("1918", 3, 5) %>% 
  pack_rows("1957", 6, 8) %>% 
  pack_rows("2020", 9, 11) %>% 
  ...
  footnote(symbol=c("Absolute number of excess deaths scaled by country population.",
                    ...
                    "Based on 8,000 posterior samples, the probability that the relative excess deaths in 2020 is larger than in 1957 is 100%, 94.3% and 99.9% in Switzerland, Sweden and Spain, respectively."))
```
...
plot_data = shown_years %>%
  left_join(results_month) %>% 
...
# plot_data %>% 
#   filter(Year == 1890 & Month == 1 & Country == "Switzerland") %>% 
#   mutate(plot1 = pred_total_deaths/Population*100000,
#          plot2 = excess_month/100) %>% 
#   select(pred_total_deaths, 
#          Population,
#          plot1,
#          excess_month,
#          plot2)
...
gg = ggplot(plot_data) +
  ...
  geom_ribbon(aes(x = Date, 
                  ymin = pred_total_deaths_lower/Population*100000,
                  ymax = pred_total_deaths_upper/Population*100000),
              fill= "aquamarine3", alpha = 0.5) +
  geom_line(aes(x = Date,
                y = pred_total_deaths/Population*100000),
            colour = "aquamarine4") +
  geom_line(aes(x = Date,
                y = Deaths/Population*100000),
            colour = "#EF8A62", size = 0.5) +
  geom_ribbon(aes(x = Date,
                  ymin=0,
                  ymax = (Deaths-pred_total_deaths)/Population*100000),
              fill = "darkorchid", size = 0, alpha=.4) +
  geom_point(data = shown_years,
...
ggsave("paper/Figure_3.png", dpi = 300, width = 280, height = 100, units = "mm")
```
...
plot_data <- 
  ...
  mutate(pdiff_m1 = (yearly_excess_total_deaths_m1 - yearly_excess_total_deaths_m0) / yearly_excess_total_deaths_m0,
         pdiff_m2 = (yearly_excess_total_deaths_m2 - yearly_excess_total_deaths_m0) / yearly_excess_total_deaths_m0,
         pdiff_m3 = (yearly_excess_total_deaths_m3 - yearly_excess_total_deaths_m0) / yearly_excess_total_deaths_m0,
         pdiff_m4 = (yearly_excess_total_deaths_m4 - yearly_excess_total_deaths_m0) / yearly_excess_total_deaths_m0
  ) %>% 
  
  mutate(diff_m1 = (yearly_excess_total_deaths_m1 - yearly_excess_total_deaths_m0) ,
         diff_m2 = (yearly_excess_total_deaths_m2 - yearly_excess_total_deaths_m0) ,
         diff_m3 = (yearly_excess_total_deaths_m3 - yearly_excess_total_deaths_m0) ,
         diff_m4 = (yearly_excess_total_deaths_m4 - yearly_excess_total_deaths_m0) 
  ) %>% 
  
  select(-starts_with("yearly")) %>% 
...
plot_data %>% 
  filter(Year %in% c(pandemic_years, pandemic_years - 1, pandemic_years + 1)) %>% 
  filter(str_detect(Model, "pdiff", negate = TRUE)) %>% 
  ggplot() + 
  geom_col(aes(x = factor(Year), y = Difference, fill = Model), position = "dodge2") +  
  facet_grid(vars(Country), scales = "free_y") + 
  scale_y_continuous(labels = comma) +
  xlab("Year") + ylab("Absolute difference") + 
  theme_bw()
```
...
### Absolute differences - all times

```{r echo=FALSE}
plot_data %>% 
  filter(str_detect(Model, "pdiff", negate = TRUE)) %>% 
  ggplot() + 
  geom_line(aes(x = Year, y = Difference, color = Model)) +  
  facet_grid(vars(Country), scales = "free_y") + 
  scale_y_continuous(labels = comma) +
  theme_bw()
```

### Relative differences - all times
...
### Absolute numbers of excess yearly.  
...
### Relative numbers of excess yearly.  
...
ggsave("paper/Figure_S2a.png", dpi = 300, width = 190, height = 140, units = "mm")
```
...
ggsave("paper/Figure_S2b.png", dpi = 300, width = 190, height = 140, units = "mm")
```
...
plot_data <- results_month %>% 
  ...
  filter(Year %in% plot_years) %>% 
  mutate(Difference = ifelse(excess_month > 0, "More", "Less"),
         Country = fct_relevel(factor(Country), 
                               "Switzerland", "Sweden", "Spain")) %>% 
  group_by(Country, Pandemic) %>% 
  mutate(time = row_number()) %>% 
...
plot_data %>% 
  ...
  geom_rect(aes(xmin = 13, xmax = 24,
                ymin = -Inf, ymax = Inf),
            fill = "grey90", alpha = 0.7, inherit.aes = FALSE) +
  geom_col(aes(y = excess_month, fill = Difference), size = 0.5) +
  facet_grid(vars(Country), vars(Pandemic), scales = "free") + 
  scale_x_continuous(limits = c(min(plot_data$time)-1, max(plot_data$time)+1),
                     breaks = c(1, 13, 24, 36, 48), 
                     labels = c("-12m", "Start", "End", "+12m", "+24m")) +
  xlab("") + 
  ylab("Excess deaths per month") +
  # ggtitle("Excess deaths per month") +
  scale_y_continuous() +
  scale_fill_manual(values = c("#67A9CF", "#EF8A62")) +  
  guides(fill = guide_legend(reverse = TRUE)) +
  theme_bw() +
...
ggsave("paper/Figure_S4.png", dpi = 300, width = 280, height = 100, units = "mm")
```
...
ggsave("paper/Figure_S5a.png", dpi = 300, width = 280, height = 100, units = "mm")
```
...
ggsave("paper/Figure_S5b.png", dpi = 300, width = 280, height = 100, units = "mm")
```
...
ggsave("paper/Figure_S6a.png", dpi = 300, width = 297, height = 210, units = "mm")
```
...
ggsave("paper/Figure_S6b.png", dpi = 300, width = 297, height = 210, units = "mm")
```
