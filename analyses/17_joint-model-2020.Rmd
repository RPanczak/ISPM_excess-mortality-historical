...
ch_pop_age_sex_2019 <- read_rds("data/mortality_org/hmd_ch_pop_age_sex.Rds") %>% 
  filter((Year >= 2015 & Year <= 2019)) %>% 
  dplyr::select(-Country,-Total) %>%
  pivot_longer(cols=c("Female","Male"), names_to = "Sex") %>%
  mutate(Age_cat = cut(Age, c(seq(0, 90, 10),120),
...
ch_pop_age_sex_2020 <- ch_pop_age_sex_2019 %>% 
  filter(Year == 2019) %>% 
  mutate(Year = 2020)
  
ch_pop_age_sex <- bind_rows(ch_pop_age_sex_2019, ch_pop_age_sex_2020)

rm(ch_pop_age_sex_2019, ch_pop_age_sex_2020)
...
# 
# ggplot(filter(ch_pop_age_sex, Year >= 2015 & Year <= 2019),
#        aes(x = Age_cat, y = Population)) +
#   # geom_jitter(colour = "darkgray") +
#   geom_point(colour = "darkgray") +
#   geom_point(data = filter(ch_pop_age_sex, Year == 2020), colour = "red") +
#   facet_grid(vars(Sex))
```
...
summary(expected_2020_age_nb_stan$samples,pars="exp_lin_total_deaths")[[1]] %>%
  as_tibble() %>%
  bind_cols(Year=rep(2015:2019,each=12),
            Month=rep(1:12,5)) %>%
  left_join(deaths_monthly) %>%
  ggplot() +
  geom_point(aes(x=Month,y=Deaths)) +
  geom_ribbon(aes(x=Month,ymin=`2.5%`,ymax=`97.5%`),alpha=.4) +
  geom_line(aes(x=Month,y=`50%`)) +
  facet_wrap(~Year)


tmp = group_by(deaths_yearly_age_sex,Year,Age_cat) %>%
  summarise(Deaths=sum(Deaths),Population=sum(Population))

summary(expected_2020_age_nb_stan$samples,pars="prop_lin_grouped_deaths")[[1]] %>%
  as_tibble() %>%
  bind_cols(Year=rep(2015:2019,each=10),
            Age_cat=rep(levels(tmp$Age_cat),5)) %>%
  left_join(tmp) %>%
  ggplot() +
  geom_point(aes(x=Age_cat,y=Deaths/Population)) +
  geom_pointrange(aes(x=Age_cat,y=`50%`,ymin=`2.5%`,ymax=`97.5%`),col="dodgerblue") +
  facet_wrap(~Year)
