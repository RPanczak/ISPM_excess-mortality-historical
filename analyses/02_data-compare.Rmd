...
```{r}
spain_pop <- read_xlsx("data-raw/Spain/Spain_Population.xlsx") %>% 
  remove_empty() %>% 
  clean_names() %>% 
  rename(Year = year) %>% 
...
spain_deaths_monthly <- read_xlsx("data-raw/Spain/Spain.xlsx") %>% 
  remove_empty() %>% 
  clean_names() %>% 
  rename(Year = jahr) %>% 
  pivot_longer(!Year, names_to = "Month", values_to = "deaths_month") %>% 
...
write_rds(spain_deaths_monthly, "data/Spain/spain_deaths_monthly.Rds")
write_dta(spain_deaths_monthly, "data/Spain/spain_deaths_monthly.dta")
```
...
write_rds(sweden_deaths_monthly, "data/Sweden/sweden_deaths_monthly.Rds")
write_dta(sweden_deaths_monthly, "data/Sweden/sweden_deaths_monthly.dta")
```
