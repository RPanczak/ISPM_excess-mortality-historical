...
Population until 2021 available in HMD.  

```{r include=FALSE}
ch_deaths_month %<>% 
  left_join(hmd_pop_year)
```
...
**Population 2021 assumed to be the same** since no better data are available atm.  
...
## 2021 age groups issue

@Julien tracked down some weird patterns in population estimates, particularly for 2020 in Switzerland.  
...
```{r}
hmd_pop_age_sex_agg <- hmd_pop_age_sex %>% 
...
For each year, age, country stratum, ~~poisson~~ linear model is fit using the 2015-2019 data; then prediction is made for 2020 & 2021 years. The gist of this operation is to exclude the effect of pandemic from the population counts in years affected.   

```{r}
pop_age_2020_lm <- hmd_pop_age_sex_agg %>% 
...
pop_age_2020_poi <- hmd_pop_age_sex_agg %>% 
  group_by(Country, Age_cat) %>% 
...
data <- bind_rows(
  ...
  mutate(Source = "HMD") %>% 
  bind_rows(pop_age_2020_lm %>% 
             mutate(Source = "Est lm")) %>% 
  bind_rows(pop_age_2020_poi %>% 
             mutate(Source = "Est poi"))
...
pop_yearly_age_sex <- bind_rows(
  ...
  # 2020 & 21 are already in HMD for SWE so only using HMD
  
  # 2020 & 21 are already in HMD for CHE so only using HMD
  ...
  # 2021 for ESP >> derived from 2020
  es_pop_age_2020 %>% 
    mutate(Year = as.integer(2021))
