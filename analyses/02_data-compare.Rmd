...
2021 data available from `Estimate of number of weekly deaths during the Covid-19 outbreak` [here](https://www.ine.es/en/experimental/defunciones/experimental_defunciones.htm#tablas_resultados).  
...
2021 data available from `Weekly death estimates (EDeS) during the covid-19 outbreak` [here](https://www.ine.es/en/experimental/defunciones/experimental_defunciones.htm#tablas_resultados).  
...
hmd_pop_age_agg %>% 
  ...
  filter(Year >= 2010) %>%
  ggplot(
    aes(x = Year, y = Population, fill = Year)) +
  geom_col() +
  scale_y_continuous() +
  theme_bw() +
...
For each year, age, country stratum, linear model is fit using the 2010-2019 data; then prediction is made for 2020 & 2021 years. The gist of this operation is to exclude the effect of pandemic from the population counts in years affected.   
...
For comparison against 2020 & 21 we use data from HMD (for SWE & CHE) and INE (for ESP).  

```{r}
# Pop Spain 2020 & 2021 are from: "1 Jan 2020"
es_pop_age_2020 <- 
