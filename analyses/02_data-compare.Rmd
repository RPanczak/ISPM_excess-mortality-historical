...
**Updated** historical data from [Todesfälle nach Monat und Sterblichkeit seit 1803](https://www.bfs.admin.ch/bfs/de/home/statistiken/kataloge-datenbanken/daten.assetdetail.18584915.html).  

Using data since 1877 only where monthly registration starts.  

```{r}
ch_deaths_month_alt <- pxR::read.px("data-raw/BfS/px-x-0102020206_111.px") %>% 
...
# with(ch_deaths_month_alt, table(demografisches_merkmal_und_indikator, Month))
```
...
```{r}
# summary(ch_deaths_month_alt$Deaths - ch_deaths_month$Deaths)

ch_deaths_month %>% 
  rename(Deaths_custom = Deaths) %>% 
  left_join(ch_deaths_month_alt %>% 
