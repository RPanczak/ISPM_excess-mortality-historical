...
*Note: Excluding partial data for Aug 2021.*  
...
## 2020 pops with age structure - alternative source
...
```{r}
compare <- left_join(
  ch_pop_age_sex_2020 %>% select(Age, Total) %>% rename(Total_old = Total), 
  ch_pop_age_sex_2020_alt %>% select(Age, Total) %>% rename(Total_new = Total)) %>% 
  mutate(diff = Total_old - Total_new,
         diff_perc = (Total_old - Total_new) / Total_new)
```
...
```{r}
se_deaths_age_sex_2019_2020 <- read_xlsx("data-raw/SCB/BE0101D9_20210429-200440.xlsx", 
...
write_rds(se_deaths_age_sex_2019_2020, "data/SCB/se_deaths_age_sex_2019_2020.Rds")
```
...
deaths_yearly_age_sex <- bind_rows(
  ...
  # 2019 is already in HMD for Sweden so only 2020
  se_deaths_age_sex_2019_2020 %>% 
    filter(Year == 2020),
