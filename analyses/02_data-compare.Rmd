...
library(pacman)
p_load(tidyverse, scales, readxl, haven, janitor)
...
stmf <- read_csv("data-raw/mortality_org/stmf.csv", 
                 col_types = cols(Year = col_integer(), Week = col_integer(), 
                                  Split = col_integer(), SplitSex = col_integer(), 
...
stmf %>% 
  filter(Year >= 2020) %>% 
  ggplot(aes(x = interaction(Year, Week))) +
  geom_line(aes(y = DTotal, group = CountryCode, color = CountryCode)) +
  theme_bw() +
