...
Using provisional figures - see above for source. Same as previously mean of Jan and Dec is taken, apart from `0 Jahre` age group where only figures from `31 Dec` are taken (`1st Jan` value is strangely `0` for this group).  
...
```{r}
es_2020 <- read_xlsx("data-raw/INE/Spain_2020_death.xlsx", sheet = 1) %>%
  ...
  mutate(Year = 2020,
         Deaths = as.integer(Deaths),
         Date = anytime::anydate(paste0("2020-", Month, "-01")),
         Month = as.integer(Month)) %>% 
...
es_pop_2020 <- read_xlsx("data-raw/INE/Spain_Population_age_2020.xlsx", sheet = 1) %>%
...
es_deaths_age_2019_2020 <- bind_rows(
  ...
  read_xlsx("data-raw/INE/Spain_2020_death.xlsx", sheet = 1) %>%
  gather(., Month, Deaths, 2:13, factor_key = TRUE) %>%
  mutate(Age = as.integer(Age))%>%
  group_by(Age) %>%
...
```{r echo=FALSE}
es_pop_age_2020 <- read_xlsx("data-raw/INE/Spain_Population_age_2020.xlsx", sheet = 1) %>%
  mutate(Year = as.integer(2020),
         Age = as.integer(Age),
         Population = as.integer(Population),
         Country = "Spain") %>% 
