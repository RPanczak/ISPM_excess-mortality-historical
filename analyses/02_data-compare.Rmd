...
Example of age & sex breakdown for Switzerland, with first indication of the effects of pandemics.  
...
Using data since 1877 only where *monthly reporting starts*.  
...
ch_deaths_month_web <- read.px("data-raw/BfS/px-x-0102020206_111.px") %>% 
...
# with(ch_deaths_month_web, table(demografisches_merkmal_und_indikator, Month))
```

Data delivered earlier by BfS.    
...
```{r}
# summary(ch_deaths_month_web$Deaths - ch_deaths_month$Deaths)

ch_deaths_month %>% 
  rename(Deaths_custom = Deaths) %>% 
  left_join(ch_deaths_month_web %>% 
              rename(Deaths_web = Deaths)) %>% 
  mutate(diff = Deaths_custom - Deaths_web,
         diff_p = ((Deaths_web - Deaths_custom) / Deaths_custom) *100) %>% 
  select(-Date) %>% 
  filter(diff != 0)
```
...
ch_deaths_month %>% 
  ...
  mutate(diff = Deaths_custom - Deaths_web,
         diff_p = ((Deaths_web - Deaths_custom) / Deaths_custom) *100) %>% 
  filter(Year == 2020) %>% 
  ggplot(aes(x = Date, y = diff_p)) +
  geom_col() +
  theme_bw() +
  xlab("") + ylab("Monthly % difference in number of deaths")
```
...
Note: data seem to be now publicly available on BfS website  [here](https://www.bfs.admin.ch/bfs/en/home/statistics/population.assetdetail.17664400.html) or in custom tabs [here](https://www.pxweb.bfs.admin.ch/pxweb/en/px-x-0102020206_103/px-x-0102020206_103/px-x-0102020206_103.px/).  

```{r}
ch_deaths_age_sex_2019_2020_alt <- read.px("data-raw/BfS/px-x-0102020206_103.px") %>% 
  as_tibble() %>% remove_empty() %>% clean_names() %>% 
  filter(zivilstand == "Zivilstand - Total",
         staatsangehorigkeit == "Staatsangehörigkeit - Total") %>% 
...
data <- ch_deaths_age_sex_2019_2020 %>% 
  ...
  left_join(ch_deaths_age_sex_2019_2020_alt %>% 
              select(-Female, -Male) %>% 
              rename(Total_new = Total) 
  ) %>% 
  mutate(diff = Total_new - Total_old,
         diff_p = ((Total_new - Total_old) / Total_old) *100)

sum(data$Total_old)

sum(data$diff)

ggplot(data, aes(x = diff_p)) +
  geom_histogram()
```
...
[Demographic balance of the permanent resident population, 1861-2020](https://www.bfs.admin.ch/bfs/en/home/statistics/catalogues-databases/tables.assetdetail.13707412.html). Population of Jan and Dec averaged.  
...
2021 population is simply assumed to be the same as 2020 ftm.     
...
## Comparison - HMD vs BfS  

Both datasets seem to agree when the population state from **31st Dec** is used in BfS sources.  
