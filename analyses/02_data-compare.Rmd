...
Note: Sweden has one more year of data than other two countries:   
...
This time, Sweden has slightly less data:  
...
*Note: Excluding partial data for Jul & Aug 2021.*  
...
write_rds(ch_deaths_age_sex_2019_2020, "data/BfS/ch_deaths_age_sex_2019_2020.Rds")
```

## 2021 deaths with age & sex structure
...
write_rds(ch_deaths_age_sex_2021, "data/BfS/ch_deaths_age_sex_2021.Rds")
```
...
<!-- 2020 could be simply `lm` predicted using pops from last 10 years:   -->
...
2020 data from [Provisional demographic balance of the permanent resident population 2020 by canton, citizenship (category), sex, age and demographic component](https://www.bfs.admin.ch/bfs/en/home/statistics/catalogues-databases/data.assetdetail.16224502.html).     
...
Using provisional figures - see above for source. Same as previously mean of Jan and Dec is taken, apart from `0 Jahre` age group where only figures from `31 Dec` are taken (`1st Jan` value is `0` for this group).  
...
## 2020 pops with age structure - alternative source {.tabset}

Mr Ullmann delivered custom data as an alternative source. They however cover:

> -	Data as per 31st of December of every year

```{r}
ch_pop_age_sex_2020_alt <- read_delim("data-raw/BfS/STATPOP_2010_2020_detailliert.csv", 
...
# write_rds(ch_pop_age_sex_2020_alt, "data/BfS/ch_pop_age_sex_2020_alt.Rds")
```

```{r}
compare <- left_join(
  
  # 31 Dec for everybody for fair comparison
  pxR::read.px("data-raw/BfS/px-x-0102020000_202.px") %>% 
    as_tibble() %>% 
    remove_empty(which = c("rows", "cols")) %>% 
    clean_names() %>%
    filter(
    ...
    mutate(Country = "Switzerland", 
           Year = as.integer(2020)) %>% 
    select(Age, Total) %>% rename(Total_old = Total), 
  
  ch_pop_age_sex_2020_alt %>% select(Age, Total) %>% rename(Total_new = Total)) %>% 
  mutate(diff = Total_new - Total_old,
         diff_perc = (Total_new - Total_old) / Total_new)
```

Difference with `31st Dec` provisional estimates from the Web.  
...
compare %>% 
  ...
  scale_y_continuous(labels = comma) +
  theme_bw() +
  xlab("") + ylab("Difference in population estimates across age groups")
```
...
Problem still exist when newly delivered data are used.  
...
write_rds(es_deaths_age_2019_2020, "data/INE/es_deaths_age_2019_2020.Rds")
```

## 2021 deaths with age structure
...
write_rds(es_pop_age_2020, "data/INE/es_pop_age_2020.Rds")
```
...
write_rds(se_deaths_age_sex_2020, "data/SCB/se_deaths_age_sex_2020.Rds")
```
...
deaths_monthly <- bind_rows(
  ...
  stmf_month %>% 
    filter(Country == "Spain" & Year == 2021 & Month <= 6) %>% 
    mutate(es_deaths_month %>% 
             filter(Year == 2020 & Month == 12) %>% 
             select(Population)),
  
  # NAs for second half of year
  stmf_month %>% 
    filter(Country == "Spain" & Year == 2020 & Month > 6) %>% 
    mutate(Year = 2021, 
           Deaths = NA_integer_) %>% 
    mutate(es_deaths_month %>% 
             filter(Year == 2020 & Month == 12) %>% 
  ...
  se_deaths_month %>% mutate(Country = "Sweden"),
  ...
  ch_deaths_month %>% mutate(Country = "Switzerland"),
  
  stmf_month %>% 
    filter(Country == "Switzerland" & Year == 2021 & Month <= 6) %>% 
    mutate(ch_deaths_month %>% 
             filter(Year == 2020 & Month == 12) %>% 
             select(Population)),
  
  stmf_month %>% 
    filter(Country == "Switzerland" & Year == 2020 & Month > 6) %>% 
    mutate(Year = 2021, 
           Deaths = NA_integer_) %>% 
    mutate(ch_deaths_month %>% 
             filter(Year == 2020 & Month == 12) %>% 
  ...
  # 2021 pops in STMF filled from 2020 values!
...
pop_yearly_age_sex <- bind_rows(
  ...
  # 2021 is derived from 2020 HMD for SE
  hmd_pop_age_sex %>% 
    select(-Female, -Male) %>% 
    filter(Country == "Sweden") %>% 
    filter(Year == 2020) %>% 
