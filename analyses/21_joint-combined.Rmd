...
results_month_pand <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-07-19/All_results_month_pand.Rds"), 
  read_rds("data/outputs_2021-07-19/Switzerland_results_month_pand.Rds"), 
  read_rds("data/outputs_2021-07-19/Sweden_results_month_pand.Rds"), 
  read_rds("data/outputs_2021-07-19/Spain_results_month_pand.Rds")
)) %>% 
...
results_age <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-07-15/Switzerland_results_age.Rds"), 
  read_rds("data/outputs_2021-07-16/Sweden_results_age.Rds"), 
  read_rds("data/outputs_2021-07-16/Spain_results_age.Rds")
)) %>% 
  filter(Model == "Age Serfling (Stan, NB)") %>% 
  arrange(Country, Year)
```
...
table_1 <- 
  ...
  left_join(
    ...
    results_year %>%
      select(Country, Year, excess_year, pred, lower, upper) %>%
      filter(Year %in% pandemic_year) %>%
      rename(Excess = excess_year,
             Excess_lci = lower, Excess_uci = upper)
    
  ) %>% 
  mutate(Percent_excess = 
           round((Excess / Expected) * 100, 1)) %>% 
  relocate(Year, Country, Population, Expected, Observed, Excess, Percent_excess) %>% 
...
results_month %>% 
  ...
  geom_col(aes(y = excess_month, fill = Difference)) +
  scale_x_date(date_labels = "%Y",
               breaks = c(as.Date("1860-01-01"), 
                          ...
                          as.Date("2009-01-01"),
                          as.Date("2020-01-01")),
               limits = c(as.Date("1856-01-01") - 14, 
                          as.Date("2020-12-31"))) +
