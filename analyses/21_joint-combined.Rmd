...
# # country specific
# es_results_month <- readRDS("data/es_results_month.Rds")
# es_results_year <- readRDS("data/es_results_year.Rds")
# es_results_age <- readRDS("data/es_results_age.Rds")

# together
results_month <- bind_rows(
  readRDS("data/es_results_month.Rds")
)
results_year <- bind_rows(
  readRDS("data/es_results_year.Rds")
)
results_age <- bind_rows(
  readRDS("data/es_results_age.Rds")
)

# writeRDS(results_month, "data/results_month.Rds")
# writeRDS(results_year, "data/results_year.Rds")
# writeRDS(results_age, "data/results_age.Rds")

# unlink("data/es_results_month.Rds")
# unlink("data/es_results_year.Rds")
# unlink("data/es_results_age.Rds")
```

```{r}
results_month <- read_rds("data/es_results_month.Rds") %>% 
...
results_year <- read_rds("data/es_results_year.Rds") %>% 
  filter(Model == "Age Serfling (Stan, NB)") %>% 
  mutate(Pandemic = case_when(
    (Year >= 1889 & Year <= 1892) ~ 1890,
    (Year >= 1917 & Year <= 1920) ~ 1918,
...
results_age <- read_rds("data/es_results_age.Rds") %>% 
  filter(Model == "Age Serfling (Stan, NB)") %>% 
  mutate(Pandemic = case_when(
    (Year >= 1889 & Year <= 1892) ~ 1890,
    (Year >= 1917 & Year <= 1920) ~ 1918,
...
results_month %>% 
  mutate(Y = excess_month,
         Difference = ifelse(excess_month > 0, "More", "Less")) %>% 
  ggplot(aes(x = Date)) +
...
results_year %>% 
  mutate(Y = excess_year,
         Difference = ifelse(excess_year > 0, "More", "Less")) %>% 
  ggplot(aes(x = Year)) +
  geom_hline(data = hline_pandemic, aes(yintercept = yintercept), 
...
table_1 <- results_year %>%
  select(Country, Year, Deaths, Population, pred) %>%
  filter(Year %in% pandemic_year)%>%
  group_by(Year, Country) %>%
  summarise(Expected_Mortality = round(sum(pred), 0),
            Deaths_num = sum(Deaths),
            Cum_excess_death = round(sum(Deaths)-sum(pred), 0),
...
```{r echo=FALSE}
results_month %>% 
  ggplot(aes(x = Date, y = (Deaths/Population)*10000)) +
  geom_rect(xmin = as.Date(paste0(1890, "-01-01")),
            xmax = as.Date(paste0(1890, "-12-31")),
            ymin = 0, ymax = Inf,
            fill = "#d7191c", alpha = 0.01, inherit.aes = FALSE) +
...
results_month %>% 
  ...
  mutate(deaths_pop = (Deaths/Population)*10000) %>% 
  pivot_longer(-Date) %>% 
...
results_month %>% 
  ...
  geom_line(aes(y = pred), colour = "#67A9CF") +
...
```{r echo=FALSE}
table_data <- results_month %>% 
  # filter(Country == "Spain") %>% 
  filter(Year >= 1919 & Year <= 1919) %>% 
  select(Country, Month, Deaths, fit, pred) %>% 
  mutate(fit = round(fit),
         fit_diff = fit - Deaths,
         pred = round(pred),
         pred_diff = pred - Deaths) %>% 
  relocate(fit_diff, .after = fit) %>% 
  relocate(pred_diff, .after = pred)  
...
table_data%>% 
  ...
  column_spec(5, color = ifelse(table_data$fit_diff > 0, 
                                "#EF8A62", "#67A9CF")) %>% 
  column_spec(7, color = ifelse(table_data$pred_diff > 0, 
                                "#EF8A62", "#67A9CF")) %>% 
  row_spec(1, italic = FALSE) %>% 
...
# !!! unfinished !!!
# needs 2020...
bfs_2015 <- read_rds("data/BfS/deaths_week_2010-2019.Rds") %>%
  filter(Year == 2015) %>%
  select(-Ending) %>%
...
compare <- bind_rows(
  ...
  bind_rows(
    read_rds("data/BfS/deaths_week_2010-2019.Rds"),
    read_rds("data/BfS/deaths_week_2020-2021.Rds")) %>% 
    filter(Year %in% focus_years) %>% 
    # select(-Ending) %>% 
    # mutate(
