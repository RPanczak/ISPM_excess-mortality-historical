...
table_2 %>% 
  ...
  column_spec(1, italic = TRUE) %>%
  row_spec(1, italic = FALSE) %>% 
  footnote(alphabet=c("Absolute number of excess deaths scaled by country population.",
...
hline_pandemic <- results_year %>% 
  ...
  ungroup()
vline_pandemic <- tibble(Year=c(1890,1918,1957,1968,1977,2009,2020)) 
# FIXME: check with Kaspar the additional pandemic years 1968 1977 and 2009
results_year %>% 
  left_join(pops) %>% 
  mutate(Difference = ifelse(yearly_excess_total_deaths > 0, "More than expected", "Fewer than expected"),
...
results_age %>% 
  ...
  labs(x="Age group",y="Relative excess deaths",
       colour="Country")
...
results_age %>% 
  ...
  theme(legend.position = c(.1,.8),
        ...
        axis.text.x = element_text(angle=45,hjust=1),
        plot.margin = margin(0, 0, 0, 0)) +
  labs(x="Age group",y="Absolute excess deaths per 100,000 population",
       colour="Country")
