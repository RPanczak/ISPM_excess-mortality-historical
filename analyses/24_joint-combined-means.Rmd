...
results_month <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_month.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_month.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_month.Rds")
)) %>% 
...
results_year <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_year.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_year.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_year.Rds") 
)) %>% 
  filter(Model == "Age Serfling (Stan, NB)") %>% 
...
results_age <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_age.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_age.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_age.Rds") 
)) %>% 
  filter(Model == "Age Serfling (Stan, NB)") %>% 
  arrange(Country, Year) %>% 
  mutate(across(pred:excess_year_upper, round)) %>% 
  relocate(Model, Country, Year, Age_cat, Deaths)
```

## Secondary results from unadjusted models

```{r}
results_month_global <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_month.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_month.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_month.Rds")
)) %>% 
...
results_age_global <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_age.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_age.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_age.Rds") 
)) %>% 
...
```{r}
results_month_boot <- read_rds("data/outputs_2021-11-18-local/All_results_month.Rds") %>% 
  mutate(Date = anytime::anydate(paste0(Year, "-", Month))) %>% 
  arrange(Country, Year, Month) %>% 
  mutate(across(pred:excess_month_lower, round)) %>% 
...
```{r}
results_month_last_7 <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_month_last_7_trim.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_month_last_7_trim.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_month_last_7_trim.Rds")
)) %>% 
...
results_year_last_7 <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_year_last_7_trim.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_year_last_7_trim.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_year_last_7_trim.Rds") 
)) %>% 
  filter(Model == "Age Serfling (Stan, NB, last 7)") %>% 
  arrange(Country, Year) %>% 
  mutate(across(excess_year:excess_year_upper, round)) %>% 
...
results_age_last_7 <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_age_last_7_trim.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_age_last_7_trim.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_age_last_7_trim.Rds") 
)) %>% 
  filter(Model == "Age Serfling (Stan, NB, last 7)") %>% 
  arrange(Country, Year) %>% 
  mutate(across(pred:excess_year_upper, round)) %>% 
...
results_month_pand <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_month_pand.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_month_pand.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_month_pand.Rds")
)) %>% 
...
results_year_pand <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_year_pand.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_year_pand.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_year_pand.Rds") 
)) %>% 
  filter(Model == "Age Serfling (Stan, NB, pandemic)") %>%
  arrange(Country, Year) %>% 
  mutate(across(excess_year:excess_year_upper, round)) %>% 
...
results_age_pand <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_age_pand.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_age_pand.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_age_pand.Rds") 
)) %>% 
  filter(Model == "Age Serfling (Stan, NB, pandemic)") %>%
  arrange(Country, Year) %>% 
  mutate(across(pred:excess_year_upper, round)) %>% 
...
```{r}
results_month_exp <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/All_results_month_exp.Rds"),
  read_rds("data/outputs_2021-11-18-local/Sweden_results_month_exp.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_month_exp.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_month_exp.Rds")
)) %>% 
  filter(Model == "Age Serfling (Stan, NB)") %>% 
  mutate(Date = anytime::anydate(paste0(Year, "-", Month))) %>% 
  arrange(Country, Year, Month) %>% 
...
results_year_exp <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_year_exp.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_year_exp.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_year_exp.Rds") 
)) %>% 
  filter(Model == "Age Serfling (Stan, NB)") %>% 
  arrange(Country, Year) %>% 
  mutate(across(excess_year:excess_year_upper, round)) %>% 
...
results_age_exp <- merge_mort(bind_rows(
  read_rds("data/outputs_2021-11-18-local/Sweden_results_age_exp.Rds"),
  read_rds("data/outputs_2021-11-18-local/Spain_results_age_exp.Rds"),
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_age_exp.Rds") 
)) %>% 
  filter(Model == "Age Serfling (Stan, NB)") %>% 
  arrange(Country, Year) %>% 
  mutate(across(pred:excess_year_upper, round)) %>% 
...
Using `COVID19` [package](https://cran.r-project.org/web/packages/COVID19/index.html) ([citation](https://doi.org/10.21105/joss.02376)).  

Data retrieved for `2021-11-22`.  

```{r}
covid_cases_deaths_20 <- covid19(c("Switzerland", "Sweden", "Spain"), 
                                 level = 1, 
                                 start = "2020-12-31", 
                                 end = "2020-12-31", 
                                 vintage = FALSE,
...
covid_cases_deaths_21 <- covid19(c("Switzerland", "Sweden", "Spain"), 
                                 level = 1, 
                                 start = "2021-07-01", 
                                 end = "2021-07-01", 
                                 vintage = FALSE,
...
covid_cases_deaths <- bind_rows(
  
  covid_cases_deaths_20 %<>% 
    select(id, administrative_area_level_1,
           confirmed, deaths) %>% 
    mutate(Year = 2020),
  
  covid_cases_deaths_21 %<>% 
    select(id, administrative_area_level_1,
           confirmed, deaths) %>% 
    left_join(covid_cases_deaths_20 %>% 
                rename(substract_confirmed = confirmed,
                       substract_deaths = deaths)) %>% 
    mutate(confirmed = confirmed - substract_confirmed,
           deaths = deaths - substract_deaths) %>% 
    select(-substract_deaths, -substract_confirmed) %>% 
    mutate(Year = 2021)
...
table_1 <- 
  ...
  mutate(across(Population:Expected, number, big.mark = ",")) %>% 
  mutate(across(Confirmed:Deaths, number, big.mark = ",")) %>% 
  mutate(across(Confirmed:Deaths, 
                ~replace(., is.na(.), "--"))) %>% 
...
table_2 <- 
  ...
  select(-Percent_lci, -Percent_uci, -Expected) %>% 
...
table_2 %>% 
  ...
  add_header_above(c(" " = 1, 
                     "Excess" = 2, 
                     "% excess" = 2,
                     "Absolute excess mortality rate" = 2)) %>% 
  pack_rows("1890", 1, 2) %>%
  pack_rows("1918", 3, 5) %>% 
  pack_rows("1957", 6, 8) %>% 
  pack_rows("2020", 9, 11) %>% 
...
gg = results_month %>%
  ...
  filter(!is.na(shown_years)) %>%
  mutate(Country = fct_relevel(factor(Country), "Switzerland", "Sweden", "Spain")) %>% 
  filter(!(shown_years=="2016-2021" & Year==2020)) %>%
  filter(!(shown_years=="2016-2021" & Country=="Sweden")) %>%
  filter(!(shown_years=="2016-2021" & Month>6)) %>%
...
Comparing estimates of excess from alternative methods: 
...
table_s1 <- 
  ...
  bind_rows(
    ...
    # global serfling model without CrI for yearly
    results_month_boot %>% 
      mutate(excess = Deaths - pred) %>% 
      group_by(Model, Country, Year) %>% 
      summarise(excess_year = sum(excess)) %>% 
      ungroup() %>% 
      mutate(Model = "m1"),
    
    # global serfling stan without age
    results_year_global  %>% 
      mutate(Model = "m2"),
    
    # global serfling stan with age, different time window
    results_year_last_7  %>% 
      mutate(Model = "m3"),
  ...
  relocate(Country, Year,
           excess_year_m1, 
           excess_year_lower_m1, 
           excess_year_upper_m1,
           excess_year_m2, 
           excess_year_lower_m2, 
           excess_year_upper_m2,
           excess_year_m3, 
           excess_year_lower_m3, 
           excess_year_upper_m3,
           excess_year_m4, 
           excess_year_lower_m4, 
           excess_year_upper_m4
  ) %>% 
  select(-excess_year_lower_m1, -excess_year_upper_m1) %>% 
  
  mutate(
    excess_year_m1 = number(excess_year_m1, 
                            accuracy = 5L, big.mark = ","),
    excess_year_m2 = number(excess_year_m2, 
                            accuracy = 5L, big.mark = ","),
    excess_year_m3 = number(excess_year_m3, 
                            accuracy = 5L, big.mark = ","),
    excess_year_m4 = number(excess_year_m4, 
                            accuracy = 5L, big.mark = ",")
  ) %>% 
  
  mutate(
    excess_year_m2_cri = str_c("(", 
                               excess_year_lower_m2, 
                               "; ",
                               excess_year_upper_m2, 
                               ")")
  ) %>% 
  relocate(excess_year_m2_cri, .after = excess_year_m2) %>% 
  select(-excess_year_lower_m2, -excess_year_upper_m2) %>% 
  
  mutate(
    excess_year_m3_cri = str_c("(", 
                               number(excess_year_lower_m3,
                                      accuracy = 5L, big.mark = ","), 
                               "; ",
                               number(excess_year_upper_m3,
                                      accuracy = 5L, big.mark = ","), 
                               ")")
  ) %>% 
  relocate(excess_year_m3_cri, .after = excess_year_m3) %>% 
  select(-excess_year_lower_m3, -excess_year_upper_m3) %>% 
  
  mutate(
    excess_year_m4_cri = str_c("(", 
                               number(excess_year_lower_m4,
                                      accuracy = 5L, big.mark = ","), 
                               "; ",
                               number(excess_year_upper_m4,
                                      accuracy = 5L, big.mark = ","), 
                               ")")
  ) %>% 
  relocate(excess_year_m4_cri, .after = excess_year_m4) %>% 
  select(-excess_year_lower_m4, -excess_year_upper_m4) %>% 
  mutate(across(excess_year_m4:excess_year_m4_cri, 
                ~replace(., is.na(.), "--"))) 
```
...
table_s1 %>% 
  ...
  add_header_above(c(" " = 1, 
                     "Poisson Serfling" = 1, 
                     "NB Serfling, no age" = 2, 
                     "NB Serfling, age, alt time" = 2,  
                     "NB Serfling, age, exp pops" = 2)) %>%   
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE) %>% 
  column_spec(1, italic = TRUE) %>%
  row_spec(1, italic = FALSE) %>% 
...
```{r echo=FALSE}
bind_rows(
  read_rds("data/outputs_2021-11-18-local/All_results_month.Rds"), 
  read_rds("data/outputs_2021-11-18-local/Switzerland_results_month.Rds"), 
  read_rds("data/outputs_2021-11-18-local/Sweden_results_month.Rds"), 
  read_rds("data/outputs_2021-11-18-local/Spain_results_month.Rds")
) %>%  
