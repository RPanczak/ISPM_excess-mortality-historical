...
table_s1 <- bind_rows(
  ...
  mutate(excess_stan_global = str_c("(", `excess_year_lower_Global Serfling (Stan, NB)`, ", ", `excess_year_upper_Global Serfling (Stan, NB)`, ")")) %>% 
  relocate(excess_stan_global, .after = `excess_year_Global Serfling (Stan, NB)`) %>% 
  select(-`excess_year_lower_Global Serfling (Stan, NB)`, -`excess_year_upper_Global Serfling (Stan, NB)`) %>% 
  
  mutate(excess_stan_age = str_c("(", `excess_year_lower_Age Serfling (Stan, NB)`, ", ", `excess_year_upper_Age Serfling (Stan, NB)`, ")")) %>% 
  relocate(excess_stan_age, .after = `excess_year_Age Serfling (Stan, NB)`) %>% 
  select(-`excess_year_lower_Age Serfling (Stan, NB)`, -`excess_year_upper_Age Serfling (Stan, NB)`)
```
