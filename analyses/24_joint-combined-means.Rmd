...
table_1 %>% 
  ...
  footnote(symbol = c("Model-predicted number of all-cause deaths from the five preceding years (95% credible interval, rounded to the next 100).",
                      ...
                      "First six months of the year only included in the analyses."))
```
...
**Note:** *the varying scales on Y axes of the panels!*
...
gg = ggplot(plot_data) +
  ...
  # geom_line(aes(x=Date,y=excess_month/100),colour="darkorchid",size=0.5) +
...
- `Age Serfling with alternative 7 day min/max time window (Stan, NB)`  
- `Age Serfling with alternative 7 day time window (Stan, NB)`  

FIXME: add 7 day results in place of obs pops
