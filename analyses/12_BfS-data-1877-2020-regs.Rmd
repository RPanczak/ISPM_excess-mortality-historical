...
bfs_web_deaths_monthly %<>% 
  ...
  mutate(deaths_flu_year = ifelse(Year %in% flu_years, NA, deaths),
...
pred_data %<>%
  mutate(fit = ilink(predict1$fit), 
         lpi1 = ilink(predict1$fit - (2 * predict1$se.fit)),
         upi1 = ilink(predict1$fit + (2 * predict1$se.fit)),
         # fit = predict2$pred, 
         lpi2 = predict2$lower,
         upi2 = predict2$upper)
```
...
pred_data %>% 
  ggplot(aes(x = Date)) +
  geom_ribbon(aes(ymin = lpi2, ymax = upi2), alpha = 0.1) + 
  geom_line(aes(y = fit)) +
