...
table_1 <- 
  ...
  relocate(Year, Country, Population, Observed, Expected, 
           starts_with("Excess"), Percent_excess) %>% 
  arrange(Year, Country)

openxlsx::write.xlsx(table_1, "paper/Table_1_mean.xlsx", 
...
vline_pandemic <- tibble(Date=c(as.Date("1860-01-01"), 
                          as.Date("1890-01-01"),
                          as.Date("1918-01-01"),
                          as.Date("1957-01-01"),
                          as.Date("1968-01-01"),
...
results_month %>% 
  ...
  ggplot(aes(x = Date)) +
  geom_vline(data=vline_pandemic,aes(xintercept=Date+365/2),size=2,colour="#ffffbf",alpha=.8) +
...
ggsave("paper/Figure_1_mean.png", dpi = 300, width = 297, height = 210, units = "mm")
```
...
<!--                          labels = label_number(accuracy = 1, suffix = "K",  -->
<!--                                                scale = 1e-3)) + -->
<!--       theme_bw() + -->
...
<!-- # ggsave("paper/Figure_2_mean.png", dpi = 300, width = 297, height = 210, units = "mm") -->
...
gg = results_month %>%
  ...
  geom_rect(aes(xmin = as.Date(paste0(focus_year, "-01-01")),
                ...
                ymin = -Inf, ymax = Inf), 
                fill = "khaki1") +
  geom_ribbon(aes(x=Date,ymin=lower,ymax=upper), fill="grey80") +
...
idx <- which(g_data_grob$layout$name %in% c("panel-1-1",
                                            "strip-t-1"))
for (i in idx) g_data_grob$grobs[[i]] <- nullGrob()

## move y axes right
# axis-l-1 needs to move 1 columns to the right
idx <- which(g_data_grob$layout$name %in% c("axis-l-1"))
...
rm(analysis_years, previous_years)

# ggsave("paper/Figure_3_mean.png", dpi = 300, width = 297, height = 210, units = "mm")
```
...
ggsave("paper/Figure_3_mean.png", dpi = 300, width = 297, height = 150, units = "mm")
```
<!-- ----------------------------------------------------- -->
...
table_s1 <- bind_rows(
  ...
  read_rds("data/outputs_2021-07-19/All_results_month.Rds") %>% 
  ...
  # remaining three
  bind_rows(
    read_rds("data/outputs_2021-07-15/Switzerland_results_year.Rds"), 
    read_rds("data/outputs_2021-07-16/Sweden_results_year.Rds"), 
    read_rds("data/outputs_2021-07-16/Spain_results_year.Rds")
  ) %>% 
  ...
  relocate(Country, Year,
           `excess_year_Global Serfling`, `excess_year_lower_Global Serfling`, `excess_year_upper_Global Serfling`,
           `excess_year_Global Serfling (Stan, NB)`, `excess_year_lower_Global Serfling (Stan, NB)`, `excess_year_upper_Global Serfling (Stan, NB)`,
           `excess_year_Age Serfling (Stan, NB)`, `excess_year_lower_Age Serfling (Stan, NB)`, `excess_year_upper_Age Serfling (Stan, NB)`) %>% 
...
openxlsx::write.xlsx(table_s1, "paper/Table_S1_mean.xlsx", 
                     row.names = FALSE, overwrite = TRUE)
```
...
results_year %>% 
  mutate(Difference = ifelse(excess_year > 0, "More", "Less")) %>% 
  ggplot(aes(x = Year)) +
  geom_hline(data = hline_pandemic, aes(yintercept = yintercept), 
  ...
  guides(fill = guide_legend(reverse = TRUE)) +
  ggtitle("Yearly number of excess deaths")

rm(hline_pandemic)

ggsave("paper/Figure_S1_mean.png", dpi = 300, width = 297, height = 210, units = "mm")
```
...
ggsave("paper/Figure_S2_mean.png", dpi = 300, width = 297, height = 210, units = "mm")
...
ggsave("paper/Figure_S3_mean.png", dpi = 300, width = 297, height = 210, units = "mm")
```
...
rm(data_year, previous, pandemic_years)
```
...
bind_rows(
  read_rds("data/outputs_2021-07-19/All_results_month.Rds"), 
  read_rds("data/outputs_2021-07-15/Switzerland_results_month.Rds"), 
  read_rds("data/outputs_2021-07-16/Sweden_results_month.Rds"), 
  read_rds("data/outputs_2021-07-16/Spain_results_month.Rds")
) %>%  
  filter(Year == 2020) %>% 
  ggplot() +
