...
hline_pandemic <- results_month %>% 
  ...
  ungroup() %>%
  mutate(Country2 = factor(Country, levels=c("Switzerland","Sweden","Spain")))
...
results_month %>% 
  ...
  facet_grid(Country2~., scales = "free_y") + 
...
results_year %>% 
  ...
  geom_vline(data = vline_pandemic, 
             aes(xintercept = Year), 
                 size = 3, colour = "#ffffbf", alpha = 0.7) +  
