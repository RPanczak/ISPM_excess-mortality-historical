...
New figure 1 following reviewers' comments.
...
hline_pandemic <- results_year %>% 
  ...
  left_join(pops) %>%
  group_by(Country) %>% 
  mutate(yintercept = excess_year/Population) %>% 
  ungroup()
...
results_year %>% 
  ...
  mutate(Difference = ifelse(excess_year > 0, "More than expected", "Less than expected"),
         rel_excess=excess_year/Population) %>% 
  ggplot(aes(x = Year)) +
  geom_vline(data = vline_pandemic, 
             aes(xintercept = Year), 
  ...
  theme(
    ...
    plot.margin = margin(0.5, 0.5, 0, 0)) +
  xlab("") + ylab("Yearly excess mortality") +
  guides(fill = guide_legend(reverse = TRUE)) 
...
If we do 2 panels, one with absolute excess deaths and the other with relative excess mortality:
...
plotdata = results_year %>% 
  filter(Year<2021) %>% 
  left_join(pops) %>% 
  mutate(Difference = ifelse(excess_year > 0, "More than expected", "Less than expected"),
         rel_excess=excess_year/Population,
         rel_excess_lower = excess_year_lower/Population,
         rel_excess_upper = excess_year_upper/Population)
...
g1 = plotdata %>% 
  ...
  geom_vline(data = vline_pandemic, 
             aes(xintercept = Year), 
             size = 3, colour = "grey90", alpha = 0.7) +  
  geom_hline(data = hline_pandemic, aes(yintercept = excess_year), 
             colour = "#984ea3", size = 0.25) +
  geom_hline(yintercept = 0, colour = "grey80") +
...
g2 = plotdata %>% 
  ...
  geom_hline(data = hline_pandemic, aes(yintercept = yintercept), 
             colour = "#984ea3", size = 0.25) +
  geom_hline(yintercept = 0, colour = "grey80") +
  geom_ribbon(aes(ymin=rel_excess_lower,ymax=rel_excess_upper),alpha=.5,fill="forestgreen") +
  geom_line(aes(y = rel_excess),size=.6) +
  # geom_pointrange(aes(y = rel_excess,ymin=rel_excess_lower,ymax=rel_excess_upper, colour = Difference)) +
  scale_x_continuous(
    breaks = c(1860, 1890, 1918, 1957, 1968, 1977, 2009, 2020),
    limits = c(1856-1, 2020+1),
    expand = c(0, 0)) +
...
cowplot::plot_grid(g1,g2,labels=LETTERS,ncol=2)
rm(vline_pandemic, hline_pandemic)
...
gg = results_month %>%
  ...
  mutate(lower=ifelse(Year==focus_year,lower,NA),
         upper=ifelse(Year==focus_year,upper,NA)) %>%
  ggplot() +
  geom_ribbon(aes(x=Month,ymin=lower/Population,ymax=upper/Population,group=Year),fill="aquamarine3",alpha=.5) +
  geom_line(aes(x=Month,y=Deaths/Population,group=Year,colour=pandyear,size=pandyear)) +
  geom_point(aes(x=Month,y=Deaths/Population,group=Year,colour=pandyear,size=pandyear)) +
  geom_point(data=shown_years,aes(x=1,y=0),colour="transparent") +
  facet_grid(Country ~ shown_years, spac="free_x") +
  scale_y_continuous(labels =scales::percent) +
  scale_x_continuous(breaks=seq(1,12,by=2),
                     labels=c("Jan.","March","May","July","Sept.","Nov.")) +
  scale_size_manual(values=c(.15,.6),guide=FALSE) +
  scale_colour_manual(values=c("grey20","#EF8A62"),guide=FALSE) +
  theme_bw() +
  theme(plot.margin = margin(0.5, 0.5, 0, 0),
        axis.text.x = element_text(angle=45,hjust=1))+
  labs(x="Month",y="Monthly mortality")
...
results_age %>% 
  ...
  ggplot() +
  geom_pointrange(aes(x=Age_cat2,
                      y=excess_year/Population,
                      ymin=excess_year_lower/Population,
                      ymax=excess_year_upper/Population,
                      colour=factor(Country))) +
  geom_line(aes(x=Age_cat2,
                y=excess_year/Population,
                colour=factor(Country))) +
  ...
  theme(legend.position = c(.1,.8),
        ...
        axis.text.x = element_text(angle=45,hjust=1),
        plot.margin = margin(0, 0, 0, 0)) +
  labs(x="Age group",y="Excess mortality",
       colour="Country")
