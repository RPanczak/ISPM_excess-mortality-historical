...
```{r}
bfs_deaths_week <- read_rds("data/BfS/bfs_deaths_week_2010-2019.Rds")  %>% 
  filter(Year >= year_from) %>% 
  filter(Year <= year) %>% 
  bind_rows(read_rds("data/BfS/bfs_deaths_week_2020-2021.Rds") %>%
              filter(Year >= year_from) %>% 
              filter(Year <= year)) %>% 
...
```{r}
reg_deaths_weekly <- bfs_deaths_week %>% 
  # mutate(NumberOfDeaths = as.integer(NumberOfDeaths)) %>% 
  filter(Year >= year_from) %>% 
  filter(Year <= year_to) 

pred_deaths_weekly <- bfs_deaths_week %>% 
  mutate(NumberOfDeaths = as.integer(NumberOfDeaths)) %>% 
  # filter(Year >= year_from)
  filter(Year == year)
