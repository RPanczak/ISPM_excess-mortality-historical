...
compare <- combineSTS(list(
  Farrington = farrington_monthly, 
  Noufaily = noufaily_monthly))
...
bfs_web_deaths_monthly %>% 
  filter(Year == year) %>% 
  select(Month, deaths_month) %>% 
  rename(BfS = deaths_month) %>% 
  # mutate(Calculated = noufaily_monthly@observed - noufaily_monthly@upperbound[ ,1]) %>% 
  mutate(Calculated = noufaily_monthly@observed[1:12, ] - noufaily_monthly@control$expected[1:12 ,1]) %>% 
  ggplot(mapping = aes(x = factor(Month))) +
  geom_col(aes(y = Calculated), width = 1, fill = "#af8dc3") +
  ylim(ymin, ymax) +
  labs(x = "", y = "Excess deaths") +
  theme_bw() 
```
...
bfs_web_deaths_monthly %>% 
  ...
  select(Month, deaths_month) %>% 
  rename(BfS = deaths_month) %>% 
  # mutate(Calculated = farrington_monthly@observed - farrington_monthly@upperbound[ ,1]) %>% 
  mutate(Calculated = farrington_monthly@observed[1:12, ] - farrington_monthly@control$expected[1:12 ,1]) %>% 
  ggplot(mapping = aes(x = factor(Month))) +
  geom_col(aes(y = Calculated), width = 1, fill = "#af8dc3") +
  ylim(ymin, ymax) +
  labs(x = "", y = "Excess deaths") +
