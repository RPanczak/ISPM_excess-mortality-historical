...
subtitle: "1877-2020 monthly Swiss data & regression approach"
...
library(pacman)
p_load(tidyverse, lubridate, scales, magrittr, slider, janitor)
```
...
# till 2019 from BfS, 2020 from STMF
deaths_monthly <- bind_rows(
  read_rds("data/BfS/ch_deaths_month.Rds") %>% 
    filter(Year < 2020) %>% 
    mutate(source = "BfS"),
...
```{r}
# all data from BfS
deaths_monthly <- read_rds("data/BfS/ch_deaths_month.Rds")
```
...
for (YEAR in year_reg:year_max){
  ...
  if (YEAR == year_reg) {
    ch_expected_deaths_monthly <- pred_data
  } else {
    ch_expected_deaths_monthly <- bind_rows(ch_expected_deaths_monthly, pred_data)
  }
}

rm(predict, predict_flu_hc, predict_flu, pred_data, reg_data, YEAR)

write_rds(ch_expected_deaths_monthly, "data/ch_expected_deaths_monthly.Rds")
```
...
mort_plot <- function(plot_Year, extremes_remove = FALSE) {
  ...
  plot_data <- ch_expected_deaths_monthly %>% 
