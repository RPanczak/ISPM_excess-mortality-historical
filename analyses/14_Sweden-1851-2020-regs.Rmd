---
title: "Excess mortality: 1851-2020 monthly Swedish data & regression approach"
...
```{r}
sweden_deaths_monthly <- read_rds("data/SCB/sweden_deaths_monthly.Rds") %>% 
...
haven::write_dta(se_expected_deaths_monthly, "data/se_expected_deaths_monthly.dta")
```
...
anim_save("res/se_mort_anim.gif")

p_unload(gganimate)
```

![](../res/se_mort_anim.gif)

## Stacked

```{r echo=FALSE}
se_expected_deaths_monthly %>% 
  mutate(Year = as.integer(Year),
         Month = as.integer(Month)) %>% 
  ggplot(aes(x = Month)) +
  geom_line(aes(y = deaths, group = Year, color = Year), alpha = 0.5) + 
  scale_y_continuous(labels = comma, limits = c(4000, 17500)) +
  scale_x_continuous(breaks = seq(1, 12, 2)) +
