---
title: "Excess mortality: 2010-2020 weekly & monthly BfS data & regression approach"
...
bfs_web_deaths_weekly %<>% 
  ...
  mutate(mean_5y   = slide_dbl(NumberOfDeaths, mean, 
                               na.rm = TRUE, .before = 5, .after = -1, .complete = TRUE),
         mean_10y  = slide_dbl(NumberOfDeaths, mean, 
                               na.rm = TRUE, .before = 10, .after = -1, .complete = TRUE),
         median_5y = slide_dbl(NumberOfDeaths, mean, 
                               na.rm = TRUE, .before = 5, .after = -1, .complete = TRUE),
         median_10y = slide_dbl(NumberOfDeaths, mean, 
                                na.rm = TRUE, .before = 10, .after = -1, .complete = TRUE)) %>% 
  ungroup() %>% 
  arrange(Year, Week) %>%
...
**PS:** Notice the gap on week 53 (end 2015 / beg 2016)!
...
bfs_web_deaths_weekly %>% 
  ...
  geom_line(aes(y = NumberOfDeaths), colour = "grey40") + 
  geom_line(aes(y = Expected), colour = "#beaed4") + 
  geom_line(aes(y = mean_10y), alpha = 0.5, size = 1, colour = "firebrick") + 
  geom_line(aes(y = median_5y), colour = "firebrick") + 
...
pred_deaths_weekly %<>%
  add_pi(mz_weekly, names = c("mz_lpi", "mz_upi"), yhatName = "mz_fit", nSims = 20000) %>% 
  as_tibble()
```
