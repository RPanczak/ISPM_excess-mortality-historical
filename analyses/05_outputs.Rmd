...
table_1 %>% 
  ...
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE) %>% 
  add_header_above(c("Calendar Year and Country", "Population, n", 
                     "Deaths From All Causes, n" = 2, 
                     "COVID-19 Deaths, n$^*$" = 1)) %>% 
  pack_rows("1890", 1, 2) %>%
  pack_rows("1918", 3, 5) %>% 
  pack_rows("1957", 6, 8) %>% 
  ...
  footnote(symbol = c("Data are from the Johns Hopkins Coronavirus Resource Center",
...
table_2 <- 
  ...
  mutate(
    Percent_excess = 
      number(yearly_rel_excess_total_deaths*100, accuracy = 0.1),
    Percent_lci = 
      number(yearly_rel_excess_total_deaths_lower*100, accuracy = 0.1),
    Percent_uci = 
      number(yearly_rel_excess_total_deaths_upper*100, accuracy = 0.1)
  ) %>% 
  ...
  mutate(rate_cri = str_c("(", Rate_lci, " to ", Rate_uci, ")")) %>%   
  relocate(rate_cri, .after = Rate_excess) %>% 
...
table_2 %>% 
  ...
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE) %>% 
  add_header_above(c("Calendar Year and Country" = 1, 
...
```{r echo=FALSE}
vline_pandemic <- tibble(Year=c(1890, 1918, 1957, 1968, 1977, 2009, 2020, 2021)) 

source("R/theme_border.R")
...
results_year %>% 
  ...
  geom_vline(data = vline_pandemic, 
             aes(xintercept = Year), 
             size = 2, colour = "grey90", alpha = 0.7) +  
  geom_hline(yintercept = 0, colour = "grey80") +
  geom_col(aes(y = abs_excess_pop, fill = Difference)) +
  scale_x_continuous(
    breaks = c(1890, 1918, 1957, 1968, 1977, 2009, 2020),
    limits = c(1880-1, 2022+1),
    expand = c(0, 0)) +
  ...
  theme(
    ...
    plot.margin = margin(3, 0, 10, 0)) +
  xlab("") + ylab("Absolute Excess Deaths per 100 000 Persons, n") +
...
```{r include=FALSE}
ggsave("paper/Figure_1.png", 
       dpi = 300, width = 190, height = 140, units = "mm")
```
...
plot_data = shown_years %>%
  ...
  mutate(customlim=case_when(Country=="Switzerland" ~ 330,
                             Country=="Sweden" ~ 330,
                             Country=="Spain" ~ 900))


gg = ggplot(plot_data) +
  geom_point(aes(x=Date,y=customlim),size=.1,colour="transparent") +
  ...
  # geom_hline(aes(yintercept=0),colour="black",alpha=.4,linetype=2) +
  geom_ribbon(aes(x = Date,
                  ymin=0,
                  ymax = (Deaths-pred_total_deaths)/Population*100000),
              fill = "cornflowerblue", size = 0, alpha=.4) +
  ...
  theme(strip.background = element_blank(),
        ...
        strip.text.y = element_text(margin = margin(0, 0, 2, 0)),
        plot.margin = margin(0, 0, -10, 0),
        panel.border = theme_border(type = c("bottom", "left")),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
  labs(x = "", y = "Monthly Deaths per 100,000 Persons, n")
...
```{r include=FALSE}
ggsave(plot = gg, "paper/Figure_2.png", 
       dpi = 300, width =  190, height = 140, units = "mm")
```
...
results_age %>% 
  ...
  geom_hline(yintercept = 0, linetype = 2, colour = "grey90") +
  ...
  labs(x = "Age group, y",
       y = "Absolute Excess Deaths per\n100,000 Persons, n",
       colour = "Country")
```

```{r include=FALSE}
ggsave("paper/Figure_3.png", 
       dpi = 300, width = 190, height = 140, units = "mm")
```
...
table_s1 %>% 
  ...
  footnote(symbol=c("Serfling, Poisson, unadjusted for age",
                    ...
                    "Serfling, NB, age adjusted, alternative time window, no trim",
                    "Serfling, NB, age adjusted, alternative time window, trim")) %>% 
  as_image(file = "paper/Table_S1.png")
```

## Alternative Figure 2

```{r include=FALSE}
focus_years <- c(1890, 1918, 1957, 1968, 1977, 2009, 2020)
```

```{r echo=FALSE, ref.label=I('figure2')}
```

```{r include=FALSE}
ggsave(plot = gg, "paper/Figure_2_alt.png", dpi = 300, width =  190, height = 140, units = "mm")
```

## Alternative Figure 3

```{r echo=FALSE, ref.label=I('figure3')}
```

```{r include=FALSE}
ggsave(plot = gg, "paper/Figure_3_alt.png", dpi = 300, width =  190, height = 140, units = "mm")
```
...
results_month %>% 
  ...
  scale_x_date(date_labels = "%Y",
               breaks = seq(as.Date("1880-01-01"),
                            as.Date("2020-12-31"), "10 years"),
               limits = c(as.Date("1879-12-31"), 
                          as.Date("2021-07-01")), 
               expand = c(0.01, 0)) +
  scale_fill_manual(values = c("#67A9CF", "#EF8A62")) +  
...
hline_pandemic <- results_month %>% 
  filter(Year >= 2020) %>% 
  group_by(Country) %>% 
  summarise(yintercept = max(excess_month/Population*100000)) %>% 
  ungroup() 
...
results_month %>% 
  ...
  geom_hline(data = hline_pandemic, 
             aes(yintercept = yintercept), 
             colour = "#984ea3", size = 0.25) +
  geom_col(aes(y = excess_month/Population*100000, 
               fill = Difference, colour = Difference), width = 1) +
  geom_hline(yintercept = 0, linetype = 2, size = 0.25) +
  scale_x_date(date_labels = "%Y",
               breaks = seq(as.Date("1880-01-01"),
                            as.Date("2020-12-31"), "10 years"),
  ...
  theme(legend.position = c(.12, .95),
        legend.background = element_blank(),
        plot.margin = margin(0.5, 0.5, 0, 0))+
  labs(x = "Year",y = "Monthly excess deaths per 100,000 population", fill = NULL) +
  guides(fill = guide_legend(reverse = TRUE)) 
...
```{r include=FALSE}
ggsave("paper/Figure_S2c.png", dpi = 300, width = 190, height = 140, units = "mm")
```
