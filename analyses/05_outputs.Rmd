...
results_year %>% 
  ...
  theme(
    ...
    legend.key.size = unit(0.25, "cm"),
    plot.margin = margin(3, 0, 10, 0)) +
  xlab("") + ylab("Absolute Excess Deaths, n") +
  guides(fill = guide_legend(reverse = TRUE, nrow = 1))
```
...
gg <- plot_data %>% 
  ...
  xlab("") + 
  ylab("Excess Deaths Per Month, n") +
...
```{r include=FALSE}
ggsave(plot = gg,
       "paper/Figure_S3b.png", 
       dpi = 300, width = 297, height = 210, units = "mm")

rm(plot_years, plot_data)
```
...
results_age %>% 
  ...
  theme(legend.position = c(.85, .2),
        ...
        plot.margin = margin(0, 0, 0, 0)) +
  labs(x = "Age group, y",
       y = "Absolute Excess Deaths, %",
       colour = "Country")
```

```{r include=FALSE}
ggsave("paper/Figure_S4.png", 
       dpi = 300, width = 280, height = 100, units = "mm")
```
