---
title: "Excess mortality: 1900-2020 monthly Spanish data & regression approach"
...
```{r}
spain_deaths_monthly <- read_rds("data/INE/spain_deaths_monthly.Rds") %>% 
...
```{r}
year_from <- min(spain_deaths_monthly$Year)
...
flu_years_hc_affected <- c(seq(1918 + 1, 1918 + 10))
...
spain_deaths_monthly %>% 
  ...
  xlab("Number of deaths") + ylab("Monthly deaths") +
  theme_minimal()
```
...
for (YEAR in year_reg:year_max){
  ...
  reg_data <- spain_deaths_monthly %>% 
    filter(Year >= YEAR - 10 & Year < YEAR) 
  
  pred_data <- spain_deaths_monthly %>% 
    filter(Year == YEAR) %>%
    mutate(
      fit = NA_real_,
      lpi = NA_real_,
...
haven::write_dta(es_expected_deaths_monthly, "data/es_expected_deaths_monthly.dta")
```
...
```{r echo=FALSE}
mort_plot(2020, extremes_remove = TRUE) 
```
...
anim_save("res/es_mort_anim.gif")

p_unload(gganimate)
```

![](../res/es_mort_anim.gif)

## Stacked

```{r echo=FALSE}
es_expected_deaths_monthly %>% 
  mutate(Year = as.integer(Year),
         Month = as.integer(Month)) %>% 
  ggplot(aes(x = Month)) +
  geom_line(aes(y = deaths, group = Year, color = Year), alpha = 0.5) + 
  scale_y_continuous(labels = comma, limits = c(15000, 165000)) +
  scale_x_continuous(breaks = seq(1, 12, 2)) +
