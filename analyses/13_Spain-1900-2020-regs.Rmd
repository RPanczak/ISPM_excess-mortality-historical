...
```{r}
spain_deaths_monthly <- bind_rows(
  read_rds("data/INE/spain_deaths_monthly.Rds") %>% 
    mutate(source = "INE") %>% 
...
pop2020 <- read_rds("data/INE/spain_deaths_monthly.Rds") %>% 
  filter(Year == 2020 & Month ==12)

spain_deaths_monthly %<>% 
  mutate(pop = ifelse(Year == 2020, pop2020$population, pop))
```
...
spain_deaths_monthly %<>% 
  ...
  mutate(deaths = as.integer(round(deaths))) %>% 
  mutate(pop = as.integer(round(pop)))
```
