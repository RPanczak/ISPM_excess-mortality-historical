...
subtitle: "1851-2020 monthly Swedish data & regression approach"
...
deaths_monthly <- bind_rows(
  read_rds("data/SCB/se_deaths_month.Rds") %>% 
    mutate(source = "SCB") %>% 
    filter(Year < 2020),
...
pop2020 <- read_rds("data/SCB/sweden_deaths_month.Rds") %>% 
  filter(Year == 2020 & Month ==12)

deaths_monthly %<>% 
  mutate(Population = ifelse(Year == 2020, pop2020$Population, Population))
```

```{r}
deaths_monthly <- read_rds("data/SCB/se_deaths_month.Rds")
```
## Years
...
flu_years_hc_affected <- c(seq(1890 + 1, 1890 + year_smooth),
...
deaths_monthly %<>% 
  mutate(Year = as.integer(Year),
         ...
         Population = as.integer(round(Population))) %>%   mutate(flu_year_hc = ifelse(Year %in% flu_years_hc, 1, 0),
...
## Three pandemics {.tabset}

### 1890

```{r echo=FALSE}
deaths_monthly %>% 
  filter(Year >= 1889 & Year <= 1893) %>% 
  ggplot(aes(x = Date)) +
  geom_line(aes(y = Deaths)) + 
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(labels = comma, limits = c(0, 17500)) +
  theme_minimal() + 
  xlab("") + ylab("Deaths")
```
...
mort_plot <- function(plot_Year, extremes_remove = FALSE) {
  ...
  plot_data <- se_expected_deaths_monthly %>% 
...
```{r echo=FALSE}
mort_plot(1890, extremes_remove = TRUE) 
```
