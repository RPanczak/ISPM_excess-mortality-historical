data {
  ...
  int J; // number of weeks (52 - 53 excluded)
  int J2; // number of weeks in 2021 (26)
  vector[I] years;
  vector[J] weeks;
  vector[J2] weeks2;
...
transformed parameters {
  ...
  for(i in 1:I) 
    for(j in 1:J)
      lin[i,j] = alpha + 
        beta_year*years[i] + 
        beta_periodic[1]*sin(2*pi()*weeks[j]/(365.25/7)) + 
        beta_periodic[2]*sin(4*pi()*weeks[j]/((365.25/2)/7)) + 
        beta_periodic[3]*cos(2*pi()*weeks[j]/(365.25/7)) + 
        beta_periodic[4]*cos(4*pi()*weeks[j]/((365.25/2)/7)) + 
...
generated quantities {
  ...
  for(j in 1:J2) {
    pred_lin[j] = alpha + 
      beta_year*(years[I]+1) + 
        beta_periodic[1]*sin(2*pi()*weeks2[j]/(365.25/7)) + 
        beta_periodic[2]*sin(4*pi()*weeks2[j]/((365.25/2)/7)) + 
        beta_periodic[3]*cos(2*pi()*weeks2[j]/(365.25/7)) + 
        beta_periodic[4]*cos(4*pi()*weeks2[j]/((365.25/2)/7)) + 
